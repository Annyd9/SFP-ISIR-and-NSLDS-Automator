...
with st.sidebar:
    ...
    # NEW UPLOAD POSITION: Compact and sidebar-contained
    uploaded_file = st.file_uploader("Upload Student ISIR (.dat)", type=["dat"], label_visibility="collapsed")
...
for idx, section_name in enumerate(tab_titles):
    with tabs[idx]:
        ...
        for i, (key, meta) in enumerate(fields.items()):
            ...
            with col:
                st.markdown(f"<div style='margin-bottom:-12px; margin-top:15px;'><p style='color:{l_color}!important; opacity:1!important; font-size:18px!important; font-weight:800!important;'>{field_num}. {meta['label']}</p></div>", unsafe_allow_html=True)
